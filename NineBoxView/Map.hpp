...
class Map {
 ...
 public:
   ...
   * 无
   * */
  void InterestArea(const Vector2i& from, const Vector2i& dst,
                    set<int>& deleteArea, set<int>& addArea) {
    ...
    std::set_intersection(
        fromArea.begin(), fromArea.end(), dstArea.begin(), dstArea.end(),
        std::inserter(intersection, intersection.begin()));  // C++11

    // deleteArea =  fromArea - 交集
    std::set_difference(fromArea.begin(), fromArea.end(), intersection.begin(),
                        intersection.end(),
                        std::inserter(deleteArea, deleteArea.begin()));
    // addArea = dstArea - 交集
    std::set_difference(dstArea.begin(), dstArea.end(), intersection.begin(),
                        intersection.end(),
                        std::inserter(addArea, addArea.begin()));
   ...
   * 无
   * */
  void InterestArea(const int& fromPosX, const int& fromPosY,
                    const int& dstPosX, const int& dstPosY,
                    set<int>& deleteArea, set<int>& addArea) {
  ...
  void AddObj(const Vector2i& pos, ObjBase& obj) {
    m_MapGridVec[pos.m_y * m_ColCount + pos.m_x].AddObj(
        obj);  //近似实现类似unordered的查找效率
  }
  ...
  void RemoveObj(const ObjBase& obj) {
    m_MapGridVec[obj.GetPosY() * m_ColCount + obj.GetPosX()].RemoveObj(
        obj.GetUid());
  }
 ...
 private:
  ...
  set<int> GetAll9Grid(const Vector2i& pos) {
    ...
    int rightIndex = (curIndex + 1) <= curColIndexMax
                         ? (curIndex + 1)
                         : curColIndexMax;  //右边格子索引
    ...
    if (leftUpIndex >= 0 && (leftUpIndex >= curColIndexMin + m_ColCount &&
                             leftUpIndex <= m_MaxIndex - 1)) {
      setIndex.emplace(leftUpIndex);
    }
    int rightUpIndex = (midUpIndex + 1) <= (curColIndexMax + m_ColCount)
                           ? (midUpIndex + 1)
                           : (curColIndexMax + m_ColCount);  //右上格子索引
    ...
    int rightDownIndex = (midDownIndex + 1) <= (curColIndexMax - m_ColCount)
                             ? (midDownIndex + 1)
                             : (curColIndexMax - m_ColCount);  //右下格子索引
    if (rightDownIndex >= 0) {
